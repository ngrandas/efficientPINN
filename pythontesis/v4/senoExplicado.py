...
# 
# ### To do
# * Implementar funciones de peso para disminuir el tiempo del algoritmo
# * Explicar que son los polinomios de chebysheff
# * Hacer los mapas con polinomios de chebysheff
...
# 
# ### Introducción y justificación para la exitencia del algoritmo de colocación de puntos
# 
...
def perdidaParaRevisar():
    ...
    for j in puntosPrueba:
        i = torch.tensor([j],device=device,requires_grad =True)
        y = redDinamica(i)
        suma += (y-torch.sin(j))**2
    return suma

def revisador():
    return varPerdidaCondicionParada<0.001*len(puntosPrueba)
...
for learningRate in list(np.logspace(np.log10(5e-3),np.log10(1e-4),6)):
...
os.chdir("/home/UANDES/n.grandas/pythontesis/pythontesis/v4/RESULTADOS/graficas") # Esta linea se coloca para guardar las graficas sacadas por la optimización
...
for rondaDeOptimizacion in range(3):
    ...
    for valor in valores:
        ...
        tiemposTomados.append(-1/(1+(valor-1)**2))
        pass
    ...
    tituloGrafica = f"Caso de prueba, iteración {rondaDeOptimizacion}"
    plt.title(tituloGrafica)
    ...
    plt.plot([minimoEnRonda],[np.min(f_prueba(x_grafica))],marker = "+",label = f"minimo es {minimoEnRonda}")
    print("Minimo Encontrado",minimoEnRonda)
    print("extremos",[extremoInferiorIntervalo,extremoSuperiorIntervalo])
    plt.legend()
    plt.savefig(tituloGrafica+".png",dpi = 300)
    plt.show()
...
for variableInutil in range(3):
    ...
    for rondaDeOptimizacion in range(3):
        ...
        for valor in valores:
            ...
            redDinamica = NeuralNetworkPrueba().to(device)
            # Solución con el nuevo valor.
            solucionar(serializar=True)
            # Toma del tiempo de la solución.
            tiemposTomados.append(time.time()-solucionar())
        ...
        minimoEnRonda : np.float128 = x_grafica[np.argmin(f_prueba(x_grafica))]
        c1 = minimoEnRonda
        plt.plot(minimoEnRonda,interpolacion(minimoEnRonda),marker = "+", ms = 5)
        # Reducción del intervalo de busqueda a un 20% para la siguiente ronda de optimización.
        extremoInferiorIntervalo = minimoEnRonda - 0.2/2 * anchoIntervalo
        extremoSuperiorIntervalo = minimoEnRonda + 0.2/2 * anchoIntervalo
        print("Minimo Encontrado",minimoEnRonda)
        print("extremos",[extremoInferiorIntervalo,extremoSuperiorIntervalo])
        pickle.dump("Optimización de C1 en ronda" + str(variableInutil),archivo)
        pickle.dump(c1,archivo)
        plt.legend()
        plt.show()
        
    
    # Optimización para el parámetro C2
    ...
    for rondaDeOptimizacion in range(3):
        ...
        pickle.dump("Optimización de C2 en ronda" + str(variableInutil),archivo)
        pickle.dump(c2,archivo)

# Optimización para el parámetro sigma
    extremoInferiorIntervalo = 0.5
    extremoSuperiorIntervalo = 2
    minimoEnRonda : np.float128 = 3
    for rondaDeOptimizacion in range(3):
        ...
        pickle.dump("Optimización de C3 en ronda" + str(variableInutil),archivo)
        pickle.dump(c2,archivo)



    
    print("Minimo Encontrado para c2 : ",minimoEnRonda)
