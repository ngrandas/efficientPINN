...
def actualizarPuntosConPesos():
    ...
    promAct = limitador(min(promAct+c1/(perdidavar/len(puntos))**c2,promAct+0.3))  # acá se da el avance en promedio
...
for ronda_solucion in range(1):
    ...
    while  not termino and tiempoInicial+3600*10>time.time() :
        ...
        if i % 20 == 0:
            ...
            termino = revisador()
        if i % 200 == 0:
            filename = f"{ahora.year}-{ahora.month}-{ahora.day}-{ahora.hour}-{ahora.minute}-PROC-Epoch:{i}-{ronda_solucion}"
            plottear(filename)
        i+=1
