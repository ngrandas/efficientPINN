...
def actualizarPuntosAleatorios():
    ...
    puntosAleatorios = torch.rand(40)*xsup
...
puntosPrueba = torch.linspace(0,xsup,400,requires_grad=True)
...
def plottear(filename_):
    if True:
            ...
            ygrafica = []
            puntosGrafica = torch.linspace(0,xsup,250)
...
for ronda_solucion in range(1):
    ...
    i = 0
    termino = False
    while  not termino and tiempoInicial+60*60*110>time.time() :
