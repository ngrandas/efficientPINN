...
import os 
import datetime
os.chdir(r"/home/externo/Documents/nico/efficientPINN/pythontesis/v7/RESULTADOS/comparacionDeTiempo") # Esta linea hace que el cuaderno corra sobre el directorio deseado
...
puntosAleatorios = torch.rand(80)
def actualizarPuntosAleatorios():
    global puntosAleatorios
    puntosAleatorios = torch.rand(80)*xsup
...
puntosPrueba = torch.linspace(0,xsup,400,requires_grad=False)
...
def revisador():
    print (f'Progreso {len(puntosPrueba)/varPerdidaCondicionParada} %')
    return varPerdidaCondicionParada<0.0001*len(puntosPrueba)
...
def perdida2():
    ...
    xPi = torch.linspace(3.1415926/2,1,2,device = device) 
...
def plottear(filename_):
    if True:
            ...
            import numpy as np
            puntosGrafica = np.linspace(0,xsup+1,250)
            plt.plot(puntosGrafica,ygrafica,label = "red")
            plt.plot(puntosGrafica,np.sin(puntosGrafica),label = "referencia",linestyle="-.")
            plt.plot(xsup,np.sin(xsup),marker = '|',ms= 10, label = 'Fin del dominio')
            plt.plot([0,np.pi/2],[0,1],ms = 20,marker = '+', linewidth = 0, label = 'Condiciones de frontera')
            plt.legend()
            if not termino:
                plt.title(f"{i} epochs")
            else:
                plt.title(f"{i} epochs terminado")
...
for ronda_solucion in range(1):
    ...
    i = 0
    termino = False
    while  not termino and tiempoInicial+90*55>time.time() :
